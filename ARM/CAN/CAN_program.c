...
void CAN_voidInit(CAN_Init_t* hcan)
{
	...
	while(((CAN->MSR & CAN_MSR_SLAK) == CAN_MSR_SLAK) || ((CAN->MSR & CAN_MSR_INAK) != CAN_MSR_INAK));
...
u8 CAN_u8Transmit(CAN_msg* pTxMsg)
{
	...
	if (transmitmailbox != CAN_TXSTATUS_NOMAILBOX)
	{
		...
		//Set Data length
		CAN->sTxMailBox[transmitmailbox].TDTR = (CAN->sTxMailBox[transmitmailbox].TDTR & (~0x0f)) | pTxMsg->len ;
...
void CAN_voidReceive(CAN_msg *msg, u8 Copy_u8FIFOIndex)
{
	...
	{
		Receive_FIFO_Reg = &(CAN->RF0R);
	}
	else
	{
		Receive_FIFO_Reg = &(CAN->RF1R);
	}
	...
	if ((*Receive_FIFO_Reg & CAN_FIFO_FMP) != 0)		//check FMP bits
	{
		...
		if ((CAN->sFIFOMailBox[Copy_u8FIFOIndex].RIR & RIR_IDE) == 0)		//edited - Manar 0x4
		{
			msg->format = CAN_ID_STD;
			msg->id     = (RIR_STID & (CAN->sFIFOMailBox[Copy_u8FIFOIndex].RIR >> 21));
		}
		else
		{
			msg->format = CAN_ID_EXT;
			msg->id     = (RIR_EXID & (CAN->sFIFOMailBox[Copy_u8FIFOIndex].RIR >> 3));
		}
		...
		msg->len = (u8)(RDT1R_DLC & CAN->sFIFOMailBox[Copy_u8FIFOIndex].RDTR);
